...
\end{defn}
\begin{lstlisting}[style=matlabCode, title={Funzione errore assoluto}]
function e = errAbs(valExact, valApprox)
%ERRABS Calcola l'errore assoluto con validazione degli input.
%   E = ERRABS(VALEXACT, VALAPPROX) calcola |valExact - valApprox|.
...
\end{defn}
\begin{lstlisting}[style=matlabCode, title={Funzione errore relativo}]
function e = errRel(valExact, valApprox)
%ERRREL Calcola l'errore relativo richiamando errAbs.
%   E = ERRREL(VALEXACT, VALAPPROX) sfrutta la funzione errAbs per il
%   numeratore e normalizza rispetto al valore esatto.
    ...
    e = errAbs(valExact, valApprox) ./ abs(valExact);
end
\end{lstlisting}
\end{bxthm}
...
\begin{lstlisting}[style=matlabCode, title={Funzione cifre decimali corrette}]
function n = cifDecCorr(valExact, valApprox)
%CIFDECCORR Calcola il numero di cifre decimali corrette.
%   n = floor(-log10(2 * errAbs(valExact, valApprox)))
...
\begin{lstlisting}[style=matlabCode, title={Funzione cifre significative corrette}]
function n = cifSigCorr(valExact, valApprox)
%CIFSIGCORR Calcola il numero di cifre significative corrette.
%   n = floor(1-log10(2 * errRel(valExact, valApprox)))
...
con
$$ |\delta_{i}| \le u \le \epsilon, \quad i\in\{1,2,3,4\} $$
...
$$ Uv_{i} = e_{i}, \quad i\in\{1,\ldots,n\} $$
...
$$ a_{i,j} = \sum_{k=1}^{j-1} l_{i,k}l_{j,k} + l_{i,j}l_{j,j}, \quad i\in\{1,\ldots,n\}, \quad j=1,\dots,i-1 $$
