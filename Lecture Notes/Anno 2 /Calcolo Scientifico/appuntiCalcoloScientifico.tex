...
dove
$$ 0 < d_{1} \le 9, \quad 0 \le d_{i} \le 9, i=2,\dots,6 $$
e
$$ -38 \le q \le 38 $$
...
dove
$$ 0 < d_{1} \le 9, \quad 0 \le d_{i} \le 9, i=2,\dots,16 $$
e
$$ -308 \le q \le 308 $$
...
In seguito denoteremo con $F$ il sottoinsieme dei numeri reali rappresentabili in un calcolatore.
...
$$ r_{1} < r_{3} < r_{2} $$
Invece, il sottoinsieme $F$, oltre ad essere limitato inferiormente e superiormente, è anche bucato.
...
Si chiama invece precisione di macchina o epsilon di macchina la distanza tra due numeri macchina consecutivi.
È possibile dimostrare che l'epsilon di macchina è esattamente il doppio della unità di roundoff, cioè $\epsilon = 2u$.
Dunque parleremo indifferentemente di unità di roundoff o di precisione di macchina.
Nel caso dell'insieme $F$ (doppia precisione standard IEEE 754, con 53 bit di mantissa inclusa quella nascosta), l'unità di roundoff è uguale a:
$$ u = \frac{1}{2}2^{1-53} = 2^{-53} \approx 1.110223024625157e-016 $$
e quindi
$$ \epsilon = 2^{-52} \approx 2.220446049250313e-016 $$

La precisione di macchina $\epsilon$ rappresenta l'ampiezza del ``buco'' esistente tra i numeri macchina consecutivi:
$$ 1.000000000000000 $$
e
$$ 1.000000000000001 $$

\paragraph{Esercizi MatLab}

\textbf{Esercizio 1:} Dato il numero reale $a$ e la sua approssimazione $A$ calcolare l'errore assoluto $\Delta a$ e l'errore relativo $\delta a$ e stabilire quante sono le cifre decimali corrette e le cifre significative corrette di $A$.
...
\textbf{Esercizio 2:} $a = 0.11$ e $A = 0.11111112$
\begin{verbatim}
>> a=0.11; A=0.11111112;
>> errAss=abs(a-A)
errAss =
...
\textbf{Esercizio 3:} $a = 1000.212324$ e $A = 1000.2123231$
\begin{verbatim}
...
\textbf{Variabile eps in MatLab}
In MatLab la variabile \texttt{eps} contiene il valore
$$ \epsilon = 2^{-52} = 2.22044604925031e-016 $$
\begin{verbatim}
>> eps
ans =
...
Verifichiamo che $\epsilon$ è il buco tra i numeri macchina $1.000000000000000$ e $1.000000000000001$ utilizzando la sua caratterizzazione e ricordando che il programma fornisce come risposta 1 per vero e 0 per falso.
\begin{verbatim}
>> 1+eps>1
ans =
     1
...
con
$$ |\delta_{i}| \le u \le \epsilon, \quad i=1,2,3,4 $$
Dunque, quando si effettua una operazione macchina si commette un errore dell'ordine della precisione di macchina $\epsilon$.
...
$$ fl(\sqrt{A}) = \sqrt{A}(1+\delta), \quad |\delta| \le \epsilon $$
...
Dunque
$$ X \otimes (Y \oplus Z) \approx X \times (Y+Z)(1+\delta_{3}), \quad \delta_{3}=\delta_{1}+\delta_{2}, \quad |\delta_{3}| \le \epsilon $$
...
>> a = 0.23371258e-4
a =
     2.337125800000000e-005
>> b = 0.33678429e+2
b =
     3.367842900000000e+001
>> c = -0.33677811e+2
c =
    -3.367781100000000e+001
>> (a+b)+c
ans =
     6.413712580055631e-004
>> a+(b+c)
ans =
     6.413712580028940e-004
\end{verbatim}
...
>> a = 0.1234567
a =
     1.234567000000000e-001
>> b = 6666.325
b =
     6.666325000000000e+003
>> c = -6666.325
c =
    -6.666325000000000e+003
>> (a+b)+c
ans =
     1.234567000001334e-001
>> a+(b+c)
ans =
     1.234567000000000e-001
\end{verbatim}
...
>> b = 0.500009e+4
b =
     5.000900000000000e+003
>> c = -0.500008e+4
c =
    -5.000800000000000e+003
>> a*(b+c)
ans =
     8.000000000174623e+006
>> a*b + a*c
ans =
     8000000
\end{verbatim}
...
ans =
     0
\end{verbatim}
Osserviamo che i valori $p1$ e $p2$ risultano essere uguali anche se i corrispondenti numeri macchina non lo sono (il test $p1 == p2$ fornisce come risultato 0, cioè false). Ciò accade per un errore di visualizzazione o di conversione dal binario al decimale.
...
$$ \delta z = \frac{|z - \overline{z}|}{|z|} = \frac{|x + y - x - \Delta x - y - \Delta y|}{|x + y|} $$
$$ = \frac{|\Delta x + \Delta y|}{|x + y|} \le \frac{1}{|x + y|} \left[ |x| \frac{|\Delta x|}{|x|} + |y| \frac{|\Delta y|}{|y|} \right] $$
$$ \le \frac{\max\{|x|, |y|\}}{|x + y|} \left( \frac{|\Delta x|}{|x|} + \frac{|\Delta y|}{|y|} \right) $$
$$ = \frac{\max\{|x|, |y|\}}{|x + y|} (\delta x + \delta y). $$
...
\textbf{Esercizio 1:} Risolviamo il sistema
$$ \begin{cases} x_1 - x_2 = 1 \\ x_1 - 1.00001x_2 = 0 \end{cases} $$
...
\textbf{Esercizio 2:} Risolviamo il sistema
$$ \begin{cases} y_1 - y_2 = 1 \\ y_1 - 0.99999y_2 = 0 \end{cases} $$
Definiamo la matrice del sistema:
\begin{verbatim}
>> A = [1, -1; 1, -0.99999]
...
\textbf{Esercizio 3:} Risolviamo il seguente sistema lineare con $\alpha = 0.5 \times 10^{-11}$
$$ \begin{cases} \alpha x_1 + x_2 = 1 + \alpha \\ x_1 = 1 \end{cases} $$
...
\textbf{Esercizio 4:} Siano $a = 1.4e154$ e $b = 1.3e154$. Calcolare le espressioni equivalenti
...
$$ \sum_{j=1}^{n} a_{i,j}x_{j} = b_{i} \quad i=1,\dots,n $$
ovvero:
$$
\begin{cases}
...
\paragraph{Proprietà delle Matrici: Dense e Sparse}
\begin{itemize}
    \item \textit{Matrice Densa:} Una matrice $A \in \mathbb{R}^{n \times n}$ si dice densa se la maggior parte dei suoi elementi è non nulla.
    \item \textit{Matrice Sparsa:} Una matrice $A \in \mathbb{R}^{n \times n}$ si dice sparsa se possiede un numero di elementi non nulli dell'ordine di $n$. Per memorizzare una matrice sparsa è possibile utilizzare solo tre vettori: uno per gli elementi non nulli, uno per gli indici di riga e uno per gli indici di colonna.
\end{itemize}

\paragraph{Matrici Strutturate}
Una matrice si dice strutturata se i suoi elementi sono disposti secondo una regola nota.
\begin{itemize}
\item \textit{Matrice Diagonale:} $a_{i,j} = 0$ per $i \neq j$.
$$
\begin{pmatrix}
a_{1,1} & 0 & \dots & 0 \\
0 & a_{2,2} & \dots & \vdots \\
...
\item \textit{Matrice Triangolare Inferiore:} $a_{i,j} = 0$ per $i < j$.
$$
\begin{pmatrix}
a_{1,1} & 0 & \dots & 0 \\
...
\item \textit{Matrice Triangolare Superiore:} $a_{i,j} = 0$ per $i > j$.
$$
\begin{pmatrix}
...
\item \textit{Matrice Tridiagonale:} elementi non nulli solo sulla diagonale principale, sulla sovra-diagonale e sulla sotto-diagonale.
$$
\begin{pmatrix}
...
\item \textit{Hessenberg Inferiore:} $a_{i,j} = 0$ per $j > i+1$ (elementi nulli sopra la prima sovra-diagonale).
$$
\begin{pmatrix}
a_{1,1} & a_{1,2} & 0 & \dots & 0 \\
a_{2,1} & a_{2,2} & a_{2,3} & \ddots & \vdots \\
...
\item \textit{Hessenberg Superiore:} $a_{i,j} = 0$ per $i > j+1$ (elementi nulli sotto la prima sotto-diagonale).
$$
\begin{pmatrix}
...
\paragraph{Matrici Simmetriche e Definite Positive}
Una matrice $A$ si dice \textit{simmetrica} se coincide con la sua trasposta, cioè $A = A^{T}$.
Una matrice simmetrica $A$ si dice \textit{definita positiva} se per ogni vettore $y \in \mathbb{R}^{n}, y \neq 0$ si ha $y^{T}Ay > 0$.
\textit{Criterio di Sylvester:} Una matrice simmetrica $A \in \mathbb{R}^{n \times n}$ è definita positiva se e solo se:
$$ \det(A_{k}) > 0, \quad k=1,\dots,n $$
dove $\det(A_{k})$ è il determinante della sottomatrice principale di testa di ordine $k$.

\paragraph{Matrici a Diagonale Dominante}
Una matrice $A$ è detta a \textit{diagonale dominante per righe} se:
$$ |a_{i,i}| > \sum_{j=1, j \neq i}^{n} |a_{i,j}|, \quad i=1,\dots,n $$
È detta a \textit{diagonale dominante per colonne} se:
$$ |a_{j,j}| > \sum_{i=1, i \neq j}^{n} |a_{i,j}|, \quad j=1,\dots,n $$

\paragraph{Richiami sulle Norme Vettoriali}
Una norma vettoriale è una funzione che associa ad un vettore $x \in \mathbb{R}^{n}$ un numero reale $||x||$ con le seguenti proprietà:
1. $||x|| > 0, \forall x \neq 0$ e $||x|| = 0 \Leftrightarrow x = 0$;
2. $||cx|| = |c| ||x||, \forall c \in \mathbb{R}$;
3. $||x+y|| \le ||x|| + ||y||, \forall y \in \mathbb{R}^{n}$.

Le norme più utilizzate sono:
\begin{itemize}
    \item \textit{Norma infinito:} $||x||_{\infty} = \max_{1 \le i \le n} |x_{i}|$
    \item \textit{Norma 1:} $||x||_{1} = \sum_{i=1}^{n} |x_{i}|$
    \item \textit{Norma Euclidea (o norma 2):} $||x||_{2} = \sqrt{\sum_{i=1}^{n} |x_{i}|^{2}} = \sqrt{x^{T}x}$
\end{itemize}

\paragraph{Richiami sulle Norme Matriciali}
Una norma matriciale associa ad una matrice $A$ un numero reale $||A||$ soddisfacendo proprietà analoghe a quelle vettoriali, con l'aggiunta della proprietà di sub-moltiplicatività:
4. $||AB|| \le ||A|| ||B||, \forall B \in \mathbb{R}^{n \times n}$.

Le norme matriciali più note sono:
\begin{itemize}
    \item \textit{Norma infinito:} $||A||_{\infty} = \max_{1 \le i \le n} \sum_{j=1}^{n} |a_{i,j}|$ (massima somma per righe).
    \item \textit{Norma 1:} $||A||_{1} = \max_{1 \le j \le n} \sum_{i=1}^{n} |a_{i,j}|$ (massima somma per colonne).
    \item \textit{Norma Spettrale (o norma 2):} $||A||_{2} = \sqrt{\rho(A^{T}A)}$, dove $\rho(B)$ è il raggio spettrale di $B$ (modulo massimo degli autovalori).
    \item \textit{Norma di Frobenius:} $||A||_{F} = (\sum_{i=1}^{n}\sum_{j=1}^{n} |a_{i,j}|^{2})^{\frac{1}{2}}$.
\end{itemize}

\paragraph{Norme Compatibili e Indotte}
Una norma vettoriale e una matriciale sono \textit{compatibili} se $||Ax|| \le ||A|| ||x||$.
Una norma matriciale si dice \textit{naturale} o \textit{indotta} dalla norma vettoriale se:
...
\item \textit{Matrici di Vandermonde}
...
\begin{itemize}
    \item \textit{Metodi diretti:} In assenza di errori di arrotondamento calcolano la soluzione esatta in un numero finito di passi.
    \item \textit{Metodi iterativi:} Generano una successione infinita di vettori che converge alla soluzione.
\end{itemize}
...
Se denotiamo con $x=(x_{1}, x_{2}, \dots, x_{n})^{T}$ e $b=(b_{1}, b_{2}, \dots, b_{n})^{T}$ rispettivamente il vettore delle incognite e quello dei termini noti, e se la matrice dei coefficienti è del tipo:
$$
D = \begin{pmatrix}
a_{1,1} & 0 & 0 & \dots & 0 \\
0 & a_{2,2} & 0 & \dots & 0 \\
...
\end{pmatrix}
$$
il sistema $Dx=b$, nell'ipotesi che $D$ sia non singolare (cioè $a_{i,i} \neq 0, \forall i=1,\dots,n$), si risolve banalmente mediante le $n$ divisioni:
$$ x_{i} = \frac{b_{i}}{a_{i,i}}, \quad i=1,\dots,n $$
...
\end{pmatrix}
$$
Vogliamo fornire degli algoritmi per la risoluzione dei sistemi $Lx=b$ e $Ux=b$.
Una matrice triangolare è non singolare se e solo se i suoi elementi diagonali sono tutti diversi da zero. Infatti:
$$ \det(L) = l_{1,1}l_{2,2}\dots l_{n,n} \neq 0 \Leftrightarrow l_{i,i} \neq 0, \forall i=1,\dots,n $$
$$ \det(U) = u_{1,1}u_{2,2}\dots u_{n,n} \neq 0 \Leftrightarrow u_{i,i} \neq 0, \forall i=1,\dots,n $$
...
\paragraph{Algoritmo di Sostituzione in Avanti (Forward Substitution)}
Esaminiamo il caso della matrice triangolare inferiore $Lx=b$. Esplicitamente il sistema può scriversi come:
$$
\begin{cases}
l_{1,1}x_{1} = b_{1} \\
...
$$ \dots $$
$$ x_{i} = \frac{b_{i} - \sum_{k=1}^{i-1} l_{i,k}x_{k}}{l_{i,i}}, \quad i=2,\dots,n $$
L'algoritmo è ben posto se la matrice $L$ è non singolare.

\textit{Schema Algoritmo:}
\begin{verbatim}
x(1) = b(1) / L(1,1)
for i = 2 : n
    x(i) = b(i)
...
\paragraph{Algoritmo di Sostituzione all'Indietro (Backward Substitution)}
Si vuole risolvere il sistema $Ux=b$ dove $U$ è triangolare superiore:
$$
\begin{cases}
...
\paragraph{Calcolo dell'Inversa di una Matrice Triangolare}
...
I vettori colonna $v_{i}$ dell'inversa $V=(v_{1}, \dots, v_{n})$ di $U$ soddisfano i sistemi lineari:
$$ Uv_{i} = e_{i}, \quad i=1,\dots,n $$
dove $e_{i}$ è l'$i$-esima colonna della matrice identità.
Poiché l'inversa di una matrice triangolare superiore è anch'essa triangolare superiore, le colonne di $V$ hanno tutti gli elementi subdiagonali nulli. Sfruttando questa proprietà, indichiamo con $v_{j}' = (v_{1,j}, \dots, v_{j,j})^{T}$ il vettore di dimensione $j$ tale che:
$$ U_{j}v_{j}' = e_{j}', \quad j=1,\dots,n $$
essendo $U_{j}$ la sottomatrice principale di $U$ di ordine $j$ e $e_{j}'$ il vettore di $\mathbb{R}^{j}$ con tutte le componenti nulle fuorché l'ultima che è 1.
I sistemi sono triangolari superiori di ordine $j$ e si risolvono con l'eliminazione all'indietro.

\textit{Algoritmo di Inversione:}
Per $j=1,\dots,n$:
$$ v_{j,j} = \frac{1}{u_{j,j}} $$
$$ v_{i,j} = -\frac{1}{u_{i,i}} \sum_{k=i+1}^{j} u_{i,k}v_{k,j}, \quad \text{per } i=j-1, \dots, 1 $$
...
Una procedura analoga, basata sulla sostituzione in avanti, permette di calcolare l'inversa di una matrice triangolare inferiore.
...
Gli elementi $a_{1,1}, a_{2,2}^{(2)}, a_{3,3}^{(3)}, \dots$ che compaiono al denominatore vengono detti \textit{elementi pivot}, mentre le quantità $m_{i,k}$ sono dette \textit{moltiplicatori}.
...
\end{cases}
$$
\textit{Passo 1:} $m_{2,1}=0, m_{3,1}=-1/2, m_{4,1}=0$. Il sistema diventa:
$$
\begin{cases}
2x_{1} - x_{2} + x_{3} - 2x_{4} = 0 \\
2x_{2} - x_{4} = 1 \\
...
\end{cases}
$$
\textit{Passo 2:} $m_{3,2}=-1/4, m_{4,2}=-1$. Il sistema diventa:
$$
\begin{cases}
2x_{1} - x_{2} + x_{3} - 2x_{4} = 0 \\
2x_{2} - x_{4} = 1 \\
...
\end{cases}
$$
\textit{Passo 3:} $m_{4,3} = \frac{4}{5}$. Il sistema finale è:
$$
\begin{cases}
2x_{1} - x_{2} + x_{3} - 2x_{4} = 0 \\
2x_{2} - x_{4} = 1 \\
...
\paragraph{Algoritmo e Costo Computazionale}
L'algoritmo formale è:
Per $k=1, 2, \dots, n-1$:
$$ m_{i,k} = -\frac{a_{i,k}^{(k)}}{a_{k,k}^{(k)}}, \quad i=k+1,\dots,n $$
$$ a_{i,j}^{(k+1)} = a_{i,j}^{(k)} + m_{i,k}a_{k,j}^{(k)}, \quad j=k+1,\dots,n $$
$$ b_{i}^{(k+1)} = b_{i}^{(k)} + m_{i,k}b_{k}^{(k)}, \quad i=k+1,\dots,n $$
L'algoritmo è ben posto se e solo se tutti gli elementi pivot sono diversi da zero.

\textit{Schema Algoritmo (Matlab-like):}
\begin{verbatim}
for k = 1 : n-1
    for i = k+1 : n
...
\paragraph{Teorema (Esistenza dei Pivot)}
Sia $A \in \mathbb{R}^{n \times n}$. Gli elementi pivot $a_{k,k}^{(k)}$ sono tutti diversi da zero se e soltanto se tutte le matrici principali di testa di $A$, denotate con $A_k = (a_{i,j})_{i,j=1,\dots,k}$, sono non singolari, cioè:
...
\begin{itemize}
    \item Matrici a \textit{diagonale dominante per righe} o per colonne.
    \item Matrici \textit{simmetriche definite positive} ($A^T=A$ e $y^TAy > 0, \forall y \neq 0$).
\end{itemize}
...
\begin{itemize}
    \item \textit{Pivoting Parziale:} Al passo $k$, si sceglie come pivot l'elemento di modulo massimo nella colonna $k$-esima, al di sotto o sulla diagonale principale. Si cerca l'indice $r \ge k$ tale che:
    $$ |a_{r,k}^{(k)}| = \max_{k \le i \le n} |a_{i,k}^{(k)}| $$
    Se $r \neq k$, si scambia la riga $k$ con la riga $r$.
    \item \textit{Pivoting Totale:} Si cerca l'elemento massimo su tutta la sottomatrice attiva. Si scelgono indici $r, s \ge k$ tali che:
...
\begin{itemize}
    \item Matrici a diagonale dominante per colonne.
    \item Matrici a diagonale dominante per righe.
    \item Matrici simmetriche e definite positive.
\end{itemize}
...
\paragraph{Schema Algoritmo: Gauss con Pivoting Parziale}
\begin{verbatim}
for k = 1 : n-1
...
\paragraph{Costo Computazionale con Pivoting}
...
$$ [A|b] = [A^{(1)}|b^{(1)}], \dots, [A^{(k)}|b^{(k)}], \dots, [A^{(n)}|b^{(n)}] $$
in modo tale che $A^{(n)}$ sia triangolare superiore e $b^{(n)}$ sia il nuovo termine noto.
Le matrici della successione sono tra loro legate da una trasformazione del tipo:
$$ [A^{(k+1)}|b^{(k+1)}] = M^{(k)}[A^{(k)}|b^{(k)}], \quad k=1,\dots,n-1 $$
...
$$ b = [M^{(1)}]^{-1} \dots [M^{(n-1)}]^{-1}b^{(n)} $$
Ponendo:
$$ L = [M^{(1)}]^{-1} \dots [M^{(n-1)}]^{-1}, \quad U = A^{(n)}, \quad y = b^{(n)} $$
si ottiene la fattorizzazione:
$$ A = LU \quad \text{e} \quad b = Ly $$
...
\textit{Schema Algoritmo (senza pivoting):}
\begin{verbatim}
for k = 1 : n-1
    for i = k+1 : n
...
\end{verbatim}
\paragraph{Risoluzione di Sistemi con $LU$}
Una volta fattorizzata $A=LU$, il sistema $Ax=b$ diventa $LUx=b$. Si risolve in due passi:
...
\paragraph{Applicazioni della Fattorizzazione LU}

\paragraph{1. Calcolo del Determinante di A}
...
\paragraph{2. Risoluzione di p sistemi con la stessa matrice dei coefficienti}
Supponiamo di dover risolvere i sistemi:
$$ Ax_{1}=b_{1}, \quad Ax_{2}=b_{2}, \quad \dots, \quad Ax_{p}=b_{p} $$
o in altri termini il sistema matriciale:
$$ AX=B $$
con $A \in \mathbb{R}^{n \times n}$ e $X, B \in \mathbb{R}^{n \times p}$.
Calcolate le matrici $L$ e $U$ una sola volta, ogni sistema $Ax_{i}=b_{i}$ viene risolto risolvendo i due sistemi triangolari:
$$
\begin{cases}
Ly = b_{i} \\
Ux_{i} = y
\end{cases}, \quad i=1,\dots,p
$$
...
\paragraph{3. Calcolo dell'inversa di A}
...
$$
dove i vettori $e_{i}$ rappresentano le colonne della matrice $I$ (vettori della base canonica).
Il costo computazionale complessivo "grezzo" sarebbe $\frac{n^{3}}{3} + n^{3}$.
Tuttavia, sfruttando opportunamente la natura dei vettori $e_{i}$ (che contengono molti zeri), il costo si riduce a circa $n^{3}$.
Nel dettaglio:
\begin{itemize}
...
$$ PA = \hat{L}U \quad \text{e} \quad Pb = \hat{L}y $$
...
0 & 1 & 0 & \dots & \dots & 0 \\
\vdots & & 0 & \dots & 1 & \vdots \\
\vdots & & \vdots & 1 & \vdots & \vdots \\
\vdots & & 1 & \dots & 0 & \vdots \\
0 & \dots & \dots & \dots & 0 & 1
\end{pmatrix}
\begin{matrix}
 \\ \\ \leftarrow r \\ \vdots \\ \leftarrow s \\ \\
\end{matrix}
$$
La matrice $P_{r,s}$ è detta matrice di permutazione e gode delle seguenti proprietà:
\begin{enumerate}
    \item $det(P_{r,s}) = -1$
    \item $P_{r,s} = P_{r,s}^{T}$
    \item $P_{r,s}^{2} = I$
    \item $P_{r,s}^{-1} = P_{r,s}$
\end{enumerate}
La 1) è banale. La 2) segue dal fatto che l'elemento di posto $(r, s)$ coincide con quello di posto $(s, r)$, mentre tutti gli altri elementi non nulli giacciono sulla diagonale.
La 4) segue dalla 3). Proviamo la 3):
$$ P_{r,s}P_{r,s} = (e_1, \dots, e_{r-1}, e_s, e_{r+1}, \dots, e_{s-1}, e_r, e_{s+1}, \dots, e_n)^T (e_1, \dots, e_s, \dots, e_r, \dots, e_n) $$
$$ = e_1e_1^T + \dots + e_{r-1}e_{r-1}^T + e_s e_s^T + e_{r+1}e_{r+1}^T + \dots + e_{s-1}e_{s-1}^T + e_r e_r^T + e_{s+1}e_{s+1}^T + \dots + e_n e_n^T = I $$
dove $e_i$ è l'$i$-esimo vettore colonna della base canonica.
...
\paragraph{Schema Algoritmo Fattorizzazione LU con Pivoting}
\begin{verbatim}
for k = 1:n-1
...
\paragraph{Risoluzione di sistemi lineari mediante LU con pivoting}
Nota la fattorizzazione del tipo $PA=\hat{L}U$, per determinare la soluzione del sistema lineare $Ax=b$ si risolvono due sistemi triangolari:
$$
\begin{cases}
\hat{L}y = Pb \\
Ux = y
...
L'intera procedura (fattorizzazione $PA=\hat{L}U$ e risoluzione dei due sistemi triangolari) consta di $\frac{n^3}{3} + n^2$ operazioni aritmetiche. Ci sono poi $(n-1) + (n-2) + \dots + 1 = \frac{n(n-1)}{2}$ confronti per determinare il massimo pivot in ogni passo.
...
\paragraph{Calcolo dell'inversa di A}
Denotate con $x_1, \dots, x_n$ le colonne della matrice inversa di $A$, quest'ultima può essere calcolata risolvendo i $2n$ sistemi:
$$
\begin{cases}
\hat{L}y = Pe_i \\
Ux_i = y
...
Il costo computazionale complessivo è $\frac{n^3}{3} + n^3$ ma può essere ridotto a $n^3$ se si tiene conto dalla natura dei vettori $Pe_i$.

\subsection{Metodo di Cholesky}
\paragraph{Metodo di Cholesky}
...
Se $A$ è simmetrica, allora $U_{1} = L^{T}$, e si ha:
$$ A = LDL^{T} $$
...
$$ A = L_{1}L_{1}^{T} \quad \text{con} \quad L_{1} = LD^{\frac{1}{2}} $$
...
La fattorizzazione $LL^{T}$ può essere ottenuta attraverso la procedura di fattorizzazione LU senza pivoting con un costo pari a $n^{3}/6$, cioè circa la metà della fattorizzazione LU.
...
Posto $A=(a_{i,j})_{i,j=1,\dots,n}$ e $L=(l_{i,j})_{i,j=1,\dots,n}$, si ha:
...
$$ a_{i,j} = \sum_{k=1}^{j-1} l_{i,k}l_{j,k} + l_{i,j}l_{j,j}, \quad i=1,\dots,n, \quad j=1,\dots,i-1 $$
$$ a_{i,i} = \sum_{k=1}^{i-1} l_{i,k}^{2} + l_{i,i}^{2}, \quad i=1,\dots,n $$

Da cui si ricavano le formule per gli elementi di $L$:
$$ l_{1,1} = \sqrt{a_{1,1}} $$
$$ l_{i,j} = \frac{1}{l_{j,j}} \left( a_{i,j} - \sum_{k=1}^{j-1} l_{i,k}l_{j,k} \right), \quad i=2,\dots,n, \quad j=1,\dots,i-1 $$
$$ l_{i,i} = \sqrt{a_{i,i} - \sum_{k=1}^{i-1} l_{i,k}^{2}}, \quad i=2,\dots,n $$
...
\paragraph{Schema Algoritmo di Cholesky}
\begin{verbatim}
...
$$ \begin{cases} \epsilon x_{1} + x_{2} = 1 + \epsilon \\ x_{1} = 1 \end{cases} $$
Se scegliamo $\epsilon = 0.5 \cdot 10^{-5}$ si ha:
...
\begin{itemize}
    ...
    \item \texttt{R = chol(B)}: calcola il fattore di Cholesky della matrice simmetrica e definita positiva $B$ ($B=R^{T}R$);
...
così che
$$ \overline{x} = x_{n+1} \pm \epsilon_{n+1} $$
dove
$$ \epsilon_{n+1} = |\overline{x} - x_{n+1}| < \frac{b-a}{2^{n+1}} $$
...
\begin{bxthm}
\begin{defn}
Sia $\{x_{n}\}_{n}$ una successione convergente al valore $\overline{x}$. Poniamo $\epsilon_{n} = |\overline{x} - x_{n}|$. Se esiste un numero reale $p \ge 1$ e una costante reale positiva $C$ tale che
$$ \lim_{n \to +\infty} \frac{\epsilon_{n+1}}{\epsilon_{n}^{p}} = C $$
allora diciamo che la successione $\{x_{n}\}_{n}$ ha ordine di convergenza $p$.    
\end{defn}
\end{bxthm}

Poiché per il metodo di bisezione si ha
$$ \frac{\epsilon_{n+1}}{\epsilon_{n}} \simeq \frac{1}{2} $$
...
$$ \epsilon_{n} = \frac{1}{2} 10^{-t_{n}} $$
Allora, poiché ad ogni passo l'errore si dimezza,
$$ \frac{1}{2} 10^{-t_{n+1}} = \epsilon_{n+1} = \frac{1}{2} \epsilon_{n} = \frac{1}{4} 10^{-t_{n}} $$
ovvero
...
$$ \frac{|\alpha-\beta|}{|\alpha|} \le \frac{\epsilon}{|\alpha|} \max_{k=1,\dots,n+1} \left| \frac{a_{k}\alpha^{n-k+1}}{P'(\alpha)} \right| $$
La quantità
...
$$ \frac{|\alpha-\beta|}{|\alpha|} \le \frac{\epsilon^{1/r}}{|\alpha|} \max_{k=1,\dots,n+1} \left| \frac{r! a_{k}\alpha^{n-k+1}}{P^{(r)}(\alpha)} \right|^{1/r} $$
