...
\begin{bxthm}
\begin{defn}
Una matrice $A$ si dice \textbf{simmetrica} se coincide con la sua trasposta, cioè $A = A^{T}$.
\end{defn}
\end{bxthm}
...
\begin{bxthm}
\begin{defn}
Una matrice simmetrica $A$ si dice \textbf{definita positiva} se per ogni vettore $y \in \mathbb{R}^{n}, y \neq 0$ si ha $y^{T}Ay > 0$.
\end{defn}
\end{bxthm}
...
\begin{itemize}
    ...
    \item \textbf{Norma Euclidea (o norma 2):} $$||x||_{2} = \sqrt{\sum_{i=1}^{n} |x_{i}|^{2}} = \sqrt{x^{T}x}.$$
\end{itemize}
\end{defn}
\end{bxthm}
...
\begin{itemize}
    ...
    \item \textbf{Norma Spettrale (o norma 2):} $$||A||_{2} = \sqrt{\rho(A^{T}A)},$$ dove $\rho(B)$ è il raggio spettrale di $B$ (modulo massimo degli autovalori).
...
\paragraph{Schema Algoritmo e Costo per Iterazione}
\begin{verbatim}
...
\end{verbatim}
\paragraph{Calcolo del Costo Computazionale}
Per determinare il costo totale in termini di operazioni floating-point (flops), analizziamo le operazioni svolte per ogni riga $i$-esima.
\begin{itemize}
    \item Il ciclo interno ($k$) esegue 2 operazioni (moltiplicazione e sottrazione) ripetute $i-1$ volte.
    \item Al termine del ciclo interno viene eseguita 1 divisione.
\end{itemize}
...
\subparagraph{Implementazione Ottimizzata in MATLAB}
Nel passaggio dallo schema teorico all'implementazione pratica in ambiente MATLAB (funzione \texttt{forwardSub}), sono state introdotte alcune variazioni fondamentali per garantire efficienza e stabilità numerica:
\begin{itemize}
    \item \textbf{Vettorizzazione (Vectorization):} In MATLAB, il ciclo interno (la sommatoria $\sum l_{i,k}x_k$) è sostituito da un prodotto matriciale riga-per-colonna tra la porzione della riga $i$-esima di $L$ e la parte del vettore $x$ già calcolata:
    \begin{center}
    \texttt{sommatoria = L(i, 1:i-1) * x(1:i-1);}
    \end{center}
    Sebbene la complessità computazionale rimanga $O(n^2)$, questo approccio riduce l'overhead dell'interprete e sfrutta le librerie ottimizzate di algebra lineare (BLAS), risultando significativamente più veloce nell'esecuzione rispetto a un ciclo \texttt{for} esplicito.
    
    \item \textbf{Robustezza e Validazione:} A differenza dello pseudo-codice, la funzione reale include il blocco \texttt{arguments} per la validazione dei tipi e controlli espliciti sulle dimensioni. Inoltre, la verifica della non-singolarità non avviene controllando l'uguaglianza con zero ($L_{ii} = 0$), ma verificando che $|L_{ii}| < \texttt{eps}$ (epsilon macchina), per gestire correttamente gli errori di arrotondamento propri dell'aritmetica floating point.
    
    \item \textbf{Pre-allocazione:} Il vettore soluzione viene inizializzato staticamente con \texttt{zeros(n,1)} prima del ciclo, evitando costose riallocazioni dinamiche della memoria durante l'esecuzione.
\end{itemize}
\begin{lstlisting}[style=matlabCode, title={Funzione forwardSub}]
...
%       x - Vettore soluzione (n x 1)
    ...
    for i = 2 : n
        ...
        % Il loro prodotto matriciale (*) coincide esattamente con la sommatoria che ci serve.
...
\end{lstlisting}
Di seguito viene mostrato uno script di esempio che utilizza la funzione \texttt{forwardSub} per risolvere un sistema lineare triangolare, riportando il codice e i risultati numerici ottenuti.
\begin{lstlisting}[style=matlabCode]
% 1. Impostiamo il formato numerico
format long e

% 2. Generazione dati (Generalizzato per dimensione n)
n = 4;
L = tril(rand(n));         % Matrice triangolare inferiore
b = randi([-10, 10], n, 1); % Termine noto

disp('Matrice L:'); disp(L);
disp('Vettore b:'); disp(b);

% 3. Algoritmo di Forward Substitution (Sostituzione in avanti)
x = forwardSub(L,b);

% 4. Output e Verifica
disp('Soluzione x calcolata:');
disp(x);

% 5. Calcolo del Residuo
% Calcoliamo la norma della differenza
residuo = norm(L*x - b);
fprintf('Norma del residuo ||Lx - b||: %e\n', residuo);
\end{lstlisting}
\begin{verbatim}
Matrice L:
     4.624491592423287e-01                         0                         0                         0
     4.243490398153752e-01     7.847392947607416e-01                         0                         0
     4.609163660289640e-01     4.713571537106116e-01     4.734859929653203e-01                         0
     7.701597286086093e-01     3.576273326911794e-02     1.527212004382318e-01     7.384268399769416e-01
...
\subparagraph{Schema Algoritmo e Costo per Iterazione}
\begin{verbatim}
...
\end{verbatim}
\subparagraph{Calcolo del Costo Computazionale}
Analizziamo il costo per una generica riga $i$-esima.
\begin{itemize}
    \item Il ciclo interno su $k$ va da $i+1$ a $n$. Il numero di termini nella sommatoria è quindi $n - (i+1) + 1 = n - i$.
    \item Per ogni termine del ciclo si eseguono 2 operazioni (moltiplicazione e sottrazione).
    \item Al termine del ciclo si esegue 1 divisione.
\end{itemize}
...
Ponendo $j = n-i$ (cambio di indice per semplificare la somma), quando $i=1 \to j=n-1$ e quando $i=n \to j=0$:
$$ C(n) = \sum_{j=0}^{n-1} (2j + 1) = 2\sum_{j=0}^{n-1} j + \sum_{j=0}^{n-1} 1 $$
$$ C(n) = 2 \frac{(n-1)n}{2} + n = n^2 - n + n = n^2 \text{ flops} .$$
\subparagraph{Implementazione Ottimizzata in MATLAB}
Analogamente a quanto visto per la sostituzione in avanti, l'implementazione della funzione \texttt{backwardSub} introduce ottimizzazioni cruciali rispetto allo schema teorico:
\begin{itemize}
    \item \textbf{Vettorizzazione (Vectorization):} Invece di iterare scalarmente con un ciclo interno, si calcola la sommatoria $\sum_{k=i+1}^{n} u_{i,k}x_{k}$ mediante un prodotto matriciale (dot product). 
    In MATLAB, questo si traduce nel moltiplicare la parte di riga a destra della diagonale, \texttt{U(i, i+1:n)}, per il vettore delle soluzioni già calcolate, \texttt{x(i+1:n)}. 
    Questo approccio sfrutta l'efficienza delle operazioni su blocchi di memoria contigui.
    
    \item \textbf{Robustezza Numerica:} La funzione include la validazione degli argomenti e, soprattutto, un controllo di non-singolarità basato sulla tolleranza macchina (\texttt{eps}). Se un elemento diagonale $|u_{ii}| < \varepsilon$, l'algoritmo si arresta controllatamente per evitare divisioni instabili o per zero.
\end{itemize}
\begin{lstlisting}[style=matlabCode, title={Funzione backwardSub}]
...
% 2. Generazione dati (Generalizzato per dimensione n)
n = 4;
U = triu(rand(n));         % Matrice triangolare superiore
b = randi([-10, 10], n, 1); % Termine noto

disp('Matrice U:'); disp(U);
disp('Vettore b:'); disp(b);

% 3. Algoritmo di Backward Substitution (Sostituzione all'indietro)
...
% Calcoliamo la norma della differenza
residuo = norm(U*x - b);
fprintf('Norma del residuo ||Ux - b||: %e\n', residuo);
\end{lstlisting}
\begin{verbatim}
Matrice U:
     9.727338850797841e-01     9.541744563795431e-01     2.815015591484906e-01     5.906086529196359e-01
                         ...
                         0                         0     7.111285511803251e-01     4.755467311386607e-02
                         0                         0                         0     3.487848085100589e-01
...
Poiché l'inversa di una matrice triangolare superiore è anch'essa triangolare superiore, le colonne di $V$ hanno tutti gli elementi subdiagonali nulli. Sfruttando questa proprietà, indichiamo con $\mathbf{v}_{j}' = (\mathbf{v}_{1,j}, \dots, \mathbf{v}_{j,j})^{T}$ il vettore di dimensione $j$ tale che:
$$ U_{j}\mathbf{v}_{j}' = \mathbf{e}_{j}', \quad j\in\{1,\dots,n\}$$
essendo $U_{j}$ la sottomatrice principale di $U$ di ordine $j$ e $\mathbf{e}_{j}'$ il vettore di $\mathbb{R}^{j}$ con tutte le componenti nulle fuorché l'ultima che è 1.
I sistemi sono triangolari superiori di ordine $j$ e si risolvono con l'eliminazione all'indietro.
\paragraph{Algoritmo di Inversione}
Per $j\in\{1,\dots,n\}$:
$$ \mathbf{v}_{j,j} = \frac{1}{u_{j,j}} $$
$$ \mathbf{v}_{i,j} = -\frac{1}{u_{i,i}} \sum_{k=i+1}^{j} u_{i,k}\mathbf{v}_{k,j}, \quad \text{per } i\in\{j-1, \dots, 1\} $$
...
\begin{lstlisting}[style=matlabCode, title={Funzione gaussElim}]
...
\begin{itemize}
    ...
    \item Matrici \textbf{simmetriche definite positive} ($A^T=A$ e $y^TAy > 0, \forall y \neq 0$).
\end{itemize}
...
\begin{lstlisting}[style=matlabCode, title={Funzione gaussElimP}]
...
\begin{lstlisting}[style=matlabCode, title={Funzione gaussElimLU}]
...
\begin{enumerate}
    \item Risoluzione del sistema triangolare inferiore $Ly = b$ (sostituzione in avanti).
    \item Risoluzione del sistema triangolare superiore $Ux = y$ (sostituzione all'indietro).
\end{enumerate}
...
x = backwardSub(U, y);       % Risolve Ux = y
...
\subparagraph{1. Calcolo del Determinante di A}
...
\subparagraph{2. Risoluzione di p sistemi con la stessa matrice dei coefficienti}
...
\subparagraph{3. Calcolo dell'inversa di A}
...
\begin{enumerate}
    ...
    \item $$ P_{r,s}P_{r,s} = (\mathbf{e}_1, \dots, \mathbf{e}_{r-1}, \mathbf{e}_s, \mathbf{e}_{r+1}, \dots, \mathbf{e}_{s-1}, \mathbf{e}_r, \mathbf{e}_{s+1}, \dots, \mathbf{e}_n)^T (\mathbf{e}_1, \dots, \mathbf{e}_s, \dots, \mathbf{e}_r, \dots, \mathbf{e}_n) $$
$$ = \mathbf{e}_1\mathbf{e}_1^T + \dots + \mathbf{e}_{r-1}\mathbf{e}_{r-1}^T + \mathbf{e}_s \mathbf{e}_s^T + \mathbf{e}_{r+1}\mathbf{e}_{r+1}^T + \dots + \mathbf{e}_{s-1}\mathbf{e}_{s-1}^T + \mathbf{e}_r \mathbf{e}_r^T + \mathbf{e}_{s+1}\mathbf{e}_{s+1}^T + \dots + \mathbf{e}_n \mathbf{e}_n^T = I $$
...
[L, U, P] = gaussElimLUP(A);

% 2. Permutazione del termine noto
b_perm = P * b;

% 3. Risoluzione dei sistemi triangolari
y = forwardSub(L, b_perm);   % Risolve Ly = Pb
x = backwardSub(U, y);       % Risolve Ux = y

% 4. Verifica dell'errore
sol_ref = A\b;               % Soluzione di riferimento MATLAB
errore = norm(x - sol_ref);
...
% 1. Fattorizzazione A = LU e PA = LU
[L0, U0] = gaussElimLU(A);
[L1, U1, P] = gaussElimLUP(A);

% 2. Permutazione del termine noto
b_perm = P * b;

% 3. Risoluzione dei sistemi triangolari
y0 = forwardSub(L0, b);         % Risolve Ly = b
x0 = backwardSub(U0, y0);       % Risolve Ux = y

y1 = forwardSub(L1, b_perm);    % Risolve Ly = Pb
x1 = backwardSub(U1, y1);       % Risolve Ux = y

% 4. Verifica dell'errore
sol_ref = A\b;                  % Soluzione di riferimento MATLAB
errore0 = norm(x0 - sol_ref);
errore1 = norm(x1 - sol_ref);

disp("Errore assoluto LU:");
disp(errore0);

disp("Errore assoluto LUP:");
disp(errore1);
\end{lstlisting}
\begin{verbatim}
Errore assoluto LU:
