...
\begin{itemize}
    ...
    Gli autovalori sono $\lambda_1=2$ e, dal blocco inferiore, $\lambda_{2,3} = 3 \pm 4 \implies \{7, -1\}$.
...
\paragraph{Fattorizzazione $L{}^tL$}
...
Nel seguito si fa riferimento alla fattorizzazione di cui nel teorema precedente come alla fattorizzazione del tipo $L{}^tL$.
...
\subparagraph{Costruzione del Metodo di Cholesky}
Posto $A=(a_{i,j})_{i,j\in\{1,\dots,n\}}$ e $L=(l_{i,j})_{i,j\in\{1,\dots,n\}}$, si ha:
$$ a_{i,j} = \sum_{k=1}^{n} l_{i,k}{}^tl_{k,j} = \sum_{k=1}^{n} l_{i,k}l_{j,k} $$
...
\begin{lstlisting}[style=matlabCode, title = {Function cholDec}]
...
\subparagraph{Stabilità e Risoluzione}
...
Molte routine automatiche calcolano $R={}^tL$ anziché $L$, ma è evidente che l'algoritmo è lo stesso (con un attento uso degli indici), data la simmetria della matrice di partenza $A$. 
In tali casi scriveremo:
$$ A = {}^tRR $$
Osserviamo infine che se si vuole risolvere il sistema $A\mathbf{x}=\mathbf{b}$ mediante la fattorizzazione di Cholesky basterà, 
una volta computata la fattorizzazione, e quindi calcolata $L$ (o rispettivamente $R$), risolvere i seguenti due sistemi:
$$ L\mathbf{y} = \mathbf{b}, \quad L^{T}\mathbf{x} = \mathbf{y}$$
(oppure ${}^tR\mathbf{y} = \mathbf{b}, \quad R\mathbf{x} = \mathbf{y}$ rispettivamente).
...
Sia allora $A$ una matrice non singolare di ordine $n$ e $\mathbf{b}$ un vettore colonna di ordine $n$.
...
\begin{itemize}
    ...
    \item \textbf{Fattorizzazione di Cholesky:} Se la matrice $A$ è simmetrica e definita positiva, il comando \texttt{R = chol(A)} calcola il fattore triangolare superiore $R$ tale che $A={}^tRR$.    
...
Con la \textit{Backward Error Analysis} (BEA) (Analisi dell'errore all'indietro), si misura la perturbazione nei dati di ingresso che porta al risultato finale, ipotizzando che le operazioni siano eseguite con precisione infinita.
...
\end{itemize}
    ...
    \item Consideriamo il sistema lineare $A\mathbf{x} = \mathbf{b}$ di ordine $n = 100$, dove
    $$
    A = \left(
    \begin{array}{ccccccc}
    9 & 0 & 2 & 0 & \dots & 0 & 1 \\
...
Gli intervalli $[a_{k}, b_{k}]$, $k\in\{1,\dots,n\}$, della successione vengono determinati come segue:
...
\begin{lstlisting}[style=matlabCode, title = {Function bisec}]
...
\begin{note}
Per ogni $n \ge 0$ supponiamo che l'approssimazione $x_{n}$ di $\overline{x}$ abbia $t_{n} \in \mathbb{R}$ cifre decimali corrette, cioè
...
Una formula del tipo (1) viene detta \textbf{formula iterativa} e si dice che la successione $x_{1}, x_{2}, \dots, x_{n}, \dots$ viene costruita mediante un procedimento iterativo.
La funzione
$$ g(x) = x - \frac{F(x)}{m_{n}} $$
tale che $x_{n+1} = g(x_{n})$ viene detta \textbf{funzione di iterazione semplice} perché per la costruzione della successione $\{x_{n}\}_{n}$ utilizza solo il punto $x_{n}$.
...
Quando si implementa in maniera automatica un procedimento iterativo del tipo $x_{n+1} = g(x_{n})$, $n\in\mathbb{N}$, occorrono uno o più criteri per arrestare tale procedimento.
Fissata una tolleranza TOLL arbitrariamente piccola, i criteri di arresto usualmente utilizzati sono:
\begin{enumerate}
    \item $|F(x_{n})| < \text{TOLL}$ (tanto più è piccolo TOLL tanto più $x_{n}$ è vicino ad $\overline{x}$);
    \item $|x_{n+1} - x_{n}| < \text{TOLL}$ oppure $\frac{|x_{n+1} - x_{n}|}{|x_{n+1}|} < \text{TOLL}$ (tanto più è piccolo TOLL tanto più $x_{n}$ è vicino al limite della successione $\overline{x}$);
    \item numero delle iterazioni $<$ ITMAX, dove ITMAX è una variabile intera fissata.
\end{enumerate}
Il criterio 3 entra in gioco quando i primi due falliscono. Per questo motivo la variabile ITMAX viene anche 
detta variabile tappo. In generale è preferibile utilizzare tutti e tre i criteri, perché per particolari 
funzioni si può verificare che la convergenza sia troppo lenta oppure che i criteri 1 e/o 2 non vengano mai 
soddisfatti numericamente.
...
$$ x_{n+1} = x_{n} - \frac{F(x_{n})}{F'(x_{n})}, \quad n\in\mathbb{N}. $$
...
La formula iterativa ci permette di calcolare la successione $x_{1}, x_{2}, \dots$ a partire dal punto $x_{0}$. Ma come scegliamo il punto $x_{0}$?

\vspace{10pt}

\begin{lstlisting}[style=matlabCode, title = {Function newton}]
...
\begin{itemize}
    \item $F(a)F(b) < 0$;
    \item $F'(x) \neq 0$, $\forall x \in [a,b]$;
    \item $F''(x) \ge 0\;\lor\;F''(x) \le 0$, $\forall x \in [a,b]$;
    \item $|\frac{F(a)}{F'(a)}| < (b-a)\;\land\;|\frac{F(b)}{F'(b)}| < (b-a)$;
\end{itemize}
...
Facciamo prima vedere che la condizione 4 implica che la tangente alla curva di equazione $y=F(x)$ negli 
estremi dell'intervallo interseca l'asse $x$ all'interno di $[a,b]$. Conseguentemente, se prendiamo come 
punto iniziale $x_{0}$ un punto appartenente all'intervallo $[a,b]$ (a e b inclusi) tutti i punti della 
successione $\{x_{k}\}_{k\in\mathbb{N}}$ appartengono all'intervallo $[a,b]$.
...
Per determinare l'ordine di convergenza utilizziamo il seguente teorema:
\begin{bxthm}
\begin{thm}
...
\begin{note}
Quando la funzione $F$ ha radici multiple, abbiamo visto che da un punto di vista teorico il Metodo di Newton converge. Tuttavia, spesso si verifica che esso numericamente non converge. La stessa cosa certe volte accade quando $F$ ha radici "quasi multiple".    
\end{note}
...
\textit{Svolgimento:} Localizziamo graficamente lo zero dell'equazione trovando il punto di intersezione delle curve $y=\cos x$ e $y=4x$. 
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/primafunz.png} 
\end{figure}
Si vede che lo zero $\overline{x}$ cade nell'intervallo $\left[0, \frac{1}{2}\right]$.
...
$$ F'(x) = -\sin x - 4 \implies F'(x) < 0 \quad \forall x \in \mathbb{R} $$
$$ F''(x) = -\cos x \implies F''(x) \le 0\; \text{ se } \cos x \ge 0 $$
Dunque 
$$ F(x) \le 0 \quad \forall x \in \left[0, \frac{1}{2}\right].$$
Inoltre
...
Dunque il Metodo di Newton converge $\forall x_{0} \in \left[0, \frac{1}{2}\right]$.
\end{exmp}
...
\textit{Svolgimento:} Localizziamo graficamente lo zero dell'equazione trovando il punto di intersezione delle curve $y=e^{x}$ e $y=-x/10$. Si vede che lo zero $\overline{x}$ cade nell'intervallo $\left[-2, -\frac{3}{2}\right]$.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/secondafunz.png} 
\end{figure}
Vediamo se tale intervallo è il giusto candidato per la convergenza del Metodo di Newton.
Prima verifichiamo le condizioni agli estremi. Si ha
$$ F(-2)F(-3/2) = (-0.64665e-003) \cdot (0.73130e-003) < 0 $$
Ponendo $F(x) = e^{x} + x/10$ si ha
$$ F'(x) = e^{x} + 1/10 \implies F'(x) > 0 \;\forall x \in \mathbb{R} $$
$$ F''(x) = e^{x} \implies F''(x) > 0\; \forall x \in \mathbb{R} $$
Inoltre
...
Dunque il Metodo di Newton converge $\forall x_{0} \in \left[-2, -\frac{3}{2}\right]$.    
\end{exmp}
...
\textit{Svolgimento:} Localizziamo graficamente lo zero dell'equazione trovando il punto di intersezione delle curve $y=\log x$ e $y=-x/3$. 
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/terzafunz.png} 
\end{figure}
Si vede che lo zero $\overline{x}$ cade nell'intervallo $\left[\frac{1}{2}, 1\right]$.
Vediamo se tale intervallo è il giusto candidato per la convergenza del Metodo di Newton.
Prima verifichiamo le condizioni agli estremi. Si ha
$$ F(1/2)F(1) = (-1.57944) \cdot 1 < 0 $$
Ponendo $F(x) = x + 3 \log x$ si ha
$$ F'(x) = 1 + \frac{3}{x} \implies F'(x) > 0 \;\forall x > 0 $$
$$ F''(x) = -\frac{3}{x^{2}} \implies F''(x) < 0 \;\forall x \in \left[\frac{1}{2}, 1\right] $$
Inoltre
...
Dunque il Metodo di Newton converge $\forall x_{0} \in \left[\frac{1}{2}, 1\right]$.    
\end{exmp}
...
\begin{lstlisting}[style=matlabCode, title = {Function bisnew}]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/primaeq.png} 
\end{figure}
\end{exmp}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/secondaeq.png} 
\end{figure}
...
\paragraph{Valutazione di un polinomio in un punto}
I metodi di Bisezione e Newton per il calcolo approssimato delle readici di una equazione algebrica $P(x)=0$, richiedono la valutazione del polinomio $P$ e della sua derivata prima $P'$ 
nelle successive approssimazioni $x_0,x_1,\ldots,x_n,\ldots$ della radice $\overline{x}$.
...
\subparagraph{Algoritmo 1: Metodo Immediato}
...
\subparagraph{Algoritmo 2: Metodo con Accumulazione delle Potenze} 
...
$$ b_{1}:=a_{1} $$
$$ b_{i}:=a_{i}+\overline{x}b_{i-1}, \quad i\in\{2, 3, \dots, n+1\} $$
si ha
$$ P(\overline{x})=b_{n+1} .$$
...
\begin{note}
I numeri $b_{i}, i\in\{1, \dots, n\}$ non sono altro che i coefficienti del polinomio $Q(x)$ ottenuto come quoziente della divisione del polinomio $P(x)$ per il binomio $(x-\overline{x})$ e $P(\overline{x})$ non è altro che il resto, cioè
...
\subparagraph{Costo Computazionale}
$n$ operazioni moltiplicative e $n$ operazioni additive.

\vspace{10pt}

\begin{lstlisting}[style=matlabCode, title={Function hornerP}]
function [p, b] = hornerP(a, t)
% HORNERP Esegue la divisione sintetica di P(x) per (x-t).
%  [p, b] = HORNERP(a, t)
%
%  Input:
%    a - Vettore dei coefficienti ordinati dal grado massimo al minimo
%    t - Punto in cui valutare il polinomio (scalare).
%
%  Output:
%    p - Resto della divisione (ovvero P(t)).
%    b - Coefficienti del polinomio quoziente Q(x) di grado n-1

    % Numero di coefficienti
    m = length(a);

    % Pre-allocazione di b per velocità (grado n-1)
    b = zeros(1, m-1);

    % Il primo coefficiente del quoziente è uguale al primo di P(x)
    b(1) = a(1); 
    
    % Calcolo dei coefficienti del quoziente (Regola di Ruffini)
    for i = 2:m-1
        b(i) = b(i-1) .* t + a(i);
    end
    
    % Calcolo del resto finale
    p = b(m-1) .* t + a(m);
...
In Matlab abbiamo
\begin{lstlisting}[style=matlabCode]
% Definizione del polinomio: P(x) = 2x^9 + 8x^4 - x^3 - 1
a = [2 0 0 0 0 8 -1 0 0 -1];

% Punto di valutazione
t = 1; 

% Esegue la divisione sintetica (Ruffini)
[p, b] = hornerP(a, t);

fprintf("Resto (Valore del polinomio in %d): %d\n", t, p)
fprintf("Coefficienti del polinomio Quoziente: \n")
disp(b);
\end{lstlisting}
\begin{verbatim}
Resto (Valore del polinomio in 1): 8
...
\begin{cases}
c_{1}:=b_{1} \\
c_{i}=c_{i-1}\xi+b_{i}, \quad i\in\{2,\dots,n\}
\end{cases}
$$
si ha
$$ P'(\xi)=Q(\xi)=c_{n} $$
...
\begin{lstlisting}[style=matlabCode, title={Function hornerR}]
function p = hornerR(c, r, x)
% HORNERR Calcola le prime r derivate del polinomio P(x) nel punto x.
%   p = HORNERR(c, r, x) restituisce un vettore con i valori delle derivate.
%
%   Input:
%       c : Coefficienti del polinomio (grado decrescente).
%       r : Ordine massimo della derivata (es. 2 per derivata seconda).
%       x : Punto di valutazione.
%
%   Output:
%       p : Vettore dei risultati dove p(k+1) è la derivata k-esima.
%           p(1) = P(x), p(2) = P'(x), ..., p(r+1) = P^(r)(x).

    for i = 0 : r
        % Esegue Ruffini: ottiene il resto (val) e il quoziente aggiornato (c)
        % Nota: 'c' viene sovrascritto con i coeff. del quoziente per il prossimo giro
        [val, c] = hornerP(c, x);
        
        % Formula di Taylor inversa: P^(i)(x) = Resto * i!
        p(i+1) = val * factorial(i);
    end
...
\begin{lstlisting}[style=matlabCode, title={Function horner}]
...
\subparagraph{Condizionamento di una radice semplice}
...
$$ \frac{|\alpha-\beta|}{|\alpha|} \le \frac{\varepsilon}{|\alpha|} \max_{k\in\{1,\dots,n+1\}} \left| \frac{a_{k}\alpha^{n-k+1}}{P'(\alpha)} \right| $$
La quantità
$$ \frac{1}{|\alpha|} \max_{k\in\{1,\dots,n+1\}} \left| \frac{a_{k}\alpha^{n-k+1}}{P'(\alpha)} \right| $$
...
\subparagraph{Condizionamento di una radice multipla}
Sia $\alpha$ una generica radice di molteplicità $r>1$. Si dimostra che:
$$ \frac{|\alpha-\beta|}{|\alpha|} \le \frac{\varepsilon^{1/r}}{|\alpha|} \max_{k\in\{1,\dots,n+1\}} \left| \frac{r! a_{k}\alpha^{n-k+1}}{P^{(r)}(\alpha)} \right|^{1/r} $$
Quindi il problema è in generale mal condizionato.

\vspace{10pt}

\begin{lstlisting}[style=matlabCode, title={Function condRoot}]
...
\paragraph{Implementazione degli algoritmi per equazioni algebriche}
L'analisi svolta nei paragrafi precedenti sulla stabilità della valutazione polinomiale (Algoritmo di Horner) e sul calcolo delle derivate (Horner Generalizzato) trova la sua naturale applicazione nella ricerca delle radici.
A differenza delle routine generiche per funzioni non lineari $f(x)$ qualsiasi (viste nei capitoli precedenti), le seguenti implementazioni sono ottimizzate specificamente per equazioni algebriche della forma $P_n(x)=0$.
L'utilizzo dello schema di Horner per valutare sia $P(x)$ che $P'(x)$ garantisce non solo una maggiore robustezza numerica contro gli errori di arrotondamento, ma anche un costo computazionale ridotto ($2n$ operazioni per ottenere valore e derivata).
Di seguito presentiamo tre varianti algoritmiche:

\vspace{10pt}

\subparagraph{\texttt{bisecAlg}}
Una specializzazione del metodo di \emph{Bisezione} che utilizza l'algoritmo di Horner per la valutazione puntuale del residuo. Questo approccio garantisce la convergenza globale anche per polinomi di grado elevato, dove la valutazione diretta delle potenze $x^n$ potrebbe introdurre errori significativi.

\vspace{10pt}

\begin{lstlisting}[style=matlabCode, title={Function bisecAlg}]
...
\subparagraph{\texttt{newtonAlg}} Implementazione del metodo di \emph{Newton Modificato}, progettata per gestire radici con molteplicità $m \ge 1$. Questa routine sfrutta la divisione sintetica ripetuta per valutare simultaneamente $P(x)$ e $P'(x)$ nello stesso punto, rendendo il calcolo del passo di aggiornamento $x_{k+1} = x_k - m \frac{P(x_k)}{P'(x_k)}$ particolarmente efficiente.

\vspace{10pt}

\begin{lstlisting}[style=matlabCode, title={Function newtonAlg}]
...
\subparagraph{\texttt{bisnewAlg}} Una strategia \emph{ibrida} che combina i punti di forza dei due metodi precedenti. L'algoritmo avvia una fase di ``sgrossatura'' tramite Bisezione per avvicinarsi alla radice (entrando nel bacino di attrazione), per poi commutare automaticamente al metodo di Newton (fase di ``raffinamento'') e sfruttarne la convergenza quadratica.

\vspace{10pt}

\begin{lstlisting}[style=matlabCode, title={Function bisnewAlg}]
